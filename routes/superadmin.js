...
/**
 * Envia um e-mail com a senha temporária do administrador (via Resend).
...
async function sendAdminTemporaryPasswordEmail(admin, temporaryPassword, appUrl) {
  const subject = 'Sua Senha de Administrador do AvaliaFeiras foi Redefinida';
  const html = `
      <p>Olá, ${admin.nome},</p>
...
router.get('/login', (req, res) => {
  ...
  // Limpar query params message/error para evitar loop
  if (req.originalUrl.includes('?')) {
    const url = new URL(req.originalUrl, `http://${req.headers.host}`);
    const hasParamsToClear =
...
router.get('/dashboard', verificarSuperAdmin, async (req, res) => {
  try {
    ...
    if (activeTab === 'visao-geral') {
      ...
      dataForTab.totalSolicitacoesPendentes = await SolicitacaoAcesso.countDocuments({
        status: 'Pendente'
      });
    } else if (activeTab === 'gerenciar-escolas') {
      const escolasComDetalhes = await Promise.all(
        escolasCadastradas.map(async escola => {
      ...
      dataForTab.solicitacoes = await SolicitacaoAcesso.find({ status: 'Pendente' })
        .sort({ dataSolicitacao: 'asc' })
        .lean();
      ...
      if (selectedEscolaId && mongoose.Types.ObjectId.isValid(selectedEscolaId)) {
        ...
        if (feiraAtual) {
          ...
          const criteriosDaFeira = await Criterio.find({
            feira: feiraAtual._id,
            escolaId: selectedEscolaId
          }).lean();
          ...
          for (const projeto of projetos) {
            const avaliacoesDoProjeto = avaliacoes.filter(
              a => a.projeto && String(a.projeto) === String(projeto._id)
            );
            ...
            if (!isCompleta) {
              const assignedEvaluators = avaliadores
                .filter(
                  av =>
                    av.projetosAtribuidos &&
                    av.projetosAtribuidos.some(pa => String(pa) === String(projeto._id))
                )
                .map(av => av.nome)
                .join(', ');
          ...
          if (avaliacoesDoProjeto.length > 0) {
            ...
            avaliacoesDoProjeto.forEach(aval => {
              aval.itens.forEach(item => {
                if (
                  item.nota !== undefined &&
                  item.nota !== null &&
                  notasPorCriterio[String(item.criterio)]
                ) {
            ...
            criteriosOficiaisProjeto.forEach(crit => {
              ...
              if (notasDoCriterio.length > 0) {
                const mediaCriterio =
                  notasDoCriterio.reduce((sum, current) => sum + current, 0) /
                  notasDoCriterio.length;
      ...
      for (const escola of escolasCadastradas) {
        ...
        for (const avaliador of avaliadoresDaEscola) {
          const numProjetosAtribuidos = avaliador.projetosAtribuidos
            ? avaliador.projetosAtribuidos.length
            : 0;
            ...
            for (const projetoId of avaliador.projetosAtribuidos) {
              ...
              if (avaliacao && avaliacao.itens.length > 0) {
                ...
                if (projetoDetalhes && projetoDetalhes.feira) {
                  const criteriosDaFeira = await Criterio.find({
                    feira: projetoDetalhes.feira,
                    escolaId: escola._id
                  }).lean();

                  const totalCriteriosFeira = criteriosDaFeira.length;
                  const criteriosAvaliadosComNota = avaliacao.itens.filter(
                    item =>
                      item.nota !== undefined &&
                      item.nota !== null &&
                      item.nota >= 5 &&
      ...
      for (const escola of escolasCadastradas) {
        const projetos = await Projeto.find({ escolaId: escola._id })
          .populate('categoria')
          .lean();
        ...
        for (const proj of projetos) {
          ...
          criteriosOficiaisProjeto.forEach(crit => {
            ...
            nomesCriterios[critIdStr] = crit.nome;
          });
  ...
  try {
    ...
    if (!res.headersSent) {
      if (err.code === 11000 && err.keyPattern && err.keyPattern.nome) {
        req.flash(
          'error_msg',
          `Erro: Já existe uma escola com o nome "${err.keyValue.nome}". Por favor, use um nome diferente.`
        );
      } else {
...
router.get('/escolas/:id/editar', verificarSuperAdmin, async (req, res) => {
  try {
    ...
    if (!admin) {
      req.flash(
        'error_msg',
        'Administrador principal não encontrado para esta escola. Você pode criar um ao salvar as edições.'
      );
    }
...
router.post('/escolas/:id/editar', verificarSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!adminEscola) {
      const hashedPassword = novaSenhaAdmin
        ? await hashPassword(novaSenhaAdmin)
...
router.get('/solicitacoes', verificarSuperAdmin, async (req, res) => {
  try {
    const solicitacoes = await SolicitacaoAcesso.find({ status: 'Pendente' })
      .sort({ dataSolicitacao: 'asc' })
      .lean();
...
router.post('/solicitacoes/:id/atualizar', verificarSuperAdmin, async (req, res) => {
  try {
    ...
    await SolicitacaoAcesso.findByIdAndUpdate(
      req.params.id,
      {
        nomeEscola,
        cnpj,
...
router.post('/solicitacoes/:id/aprovar', verificarSuperAdmin, async (req, res) => {
  try {
    ...
    req.flash(
      'success_msg',
      `Solicitação de acesso aprovada com sucesso! Uma escola (${novaEscola.nome}) e um administrador foram criados. A senha provisória do administrador (${solicitacao.emailContato}) é: <strong>${senhaProvisoria}</strong>.`
    );
    ...
    try {
      ...
      console.error('Erro ao enviar e-mail de aprovação:', errEmail);
    }
...
router.post('/solicitacoes/:id/rejeitar', verificarSuperAdmin, async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Erro ao enviar e-mail de rejeição:', errEmail);
    }
